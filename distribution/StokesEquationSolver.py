...
import sys
sys.path.append("..")
from utils.utils import load_mesh,load_pretrained_model,evaluate,plot_u,DatasetFromTxt,generate_data_dirichlet,StatGrad,data_generator
from NS_msnn import FullyConnectedNet,train,global_nu
...
if __name__ == '__main__':
   # Training settings
    ...
    len_inte_data = 32*args.nbatch 
    len_bound_data = 3200*args.nbatch
    
    paramsw = list(model_u_new.parameters())
    optimizer = optim.Adam(paramsw, lr=args.lr) # 实例化求解器
    ...
    coarse_loss = 0
    Loss_reshold = 1e12
    train_data = data_generator(100,200,global_nu)
