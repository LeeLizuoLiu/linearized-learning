...
import sys
sys.path.append("..")
from utils.utils import optimWithExpDecaylr, plot_sol, MultiHeadNaive,FullyConnectedNet,load_mesh,load_pretrained_model,evaluate,plot_u,DatasetFromTxt,generate_data_dirichlet,StatGrad,data_generator
...
if __name__ == '__main__':
   # Training settings
    ...
    len_bound_data = 128*args.nbatch
    ...
    lr_adjust_step = 20
    lr = args.lr
    train_data = data_generator(50,65,global_nu)
    plot_sol_drawer = plot_sol(50,65,global_nu)
    for epoch in range(loadepochs+1, args.epochs + 1): # 循环调用train() and test()进行epoch迭代
        ...
        if epoch%lr_adjust_step==0:
            optimizer,lr = optimWithExpDecaylr(args.epochs, lr_adjust_step,lr, paramsw+paramsp) # 实例化求解器
