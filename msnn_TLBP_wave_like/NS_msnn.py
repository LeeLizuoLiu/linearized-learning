...
def train(args, model_list, device, interior_train_loader, 
          ...
          optimizer, epoch,lamda,beta,gamma,collectNorm,Epochs): # 还可添加loss_func等参数
    for i in range(3):
        model_list[i].train()  
    retloss=[]
    ...
    for batch_idx, (data, target) in enumerate(interior_train_loader):   # 从数据加载器迭代一个batch的数据
        ...
        loss_w,loss3 = ResLoss_w(x,f,divu_RHS,model_list,lamda,epoch,Epochs)
...
def ResLoss_w(x,f,divu_RHS,model_list,lamda,epoch,Epochs):
    ...
    penalty = torch.exp(-torch.square(x[:,0:1]+2.5-epoch/Epochs*6)/(2*0.01))
