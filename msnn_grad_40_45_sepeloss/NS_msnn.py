...
def train(args, model_list, device, interior_train_loader, 
          ...
          optimizer_u,optimizer_p, epoch,lamda,beta,gamma): # 还可添加loss_func等参数
    retloss=[]
    ...
    for batch_idx, (data, target) in enumerate(interior_train_loader):   # 从数据加载器迭代一个batch的数据
        ...
        loss_u,loss_p = ResLoss_up(x,bdry_x,f,divu_RHS,divf,bdry_velocity,beta,lamda,model_list,epoch)
        ...
        if batch_idx % args.log_interval == 0: # 根据设置的显式间隔输出训练日志
            logging.info('Train Epoch: {:>5d}  [{:>6d}/{} ({:3.0f}%)] Loss of u: {:.6f} Loss of p: {:.6f}'.format(
                epoch, batch_idx * len(data), len(interior_train_loader.dataset),
                100. * batch_idx / len(interior_train_loader), loss_u.item(),loss_p.item()))
        retloss=loss_u+loss_p
        retbound = 0
        retres = 0
        retcoar_loss = 0

    return retloss, lamda_temp, retbound, retres, retcoar_loss
def ResLoss_up(x,bdry_x,f,divu_RHS,divf,bdry_velocity,beta,lamda,model_list,epoch):
