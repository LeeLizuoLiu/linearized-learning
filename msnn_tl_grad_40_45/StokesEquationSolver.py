...
if __name__ == '__main__':
   # Training settings
    ...
    model_u_old =FullyConnectedNet(2,nNeuron,1).to(device)	# 实例化自定义网络模型
    model_p = FullyConnectedNet(2,nNeuron,1).to(device)	# 实例化自定义网络模型
    model_u_new =FullyConnectedNet(2,nNeuron,1).to(device)	# 实例化自定义网络模型
    ...
    for epoch in range(loadepochs+1, args.epochs+1): # 循环调用train() and test()进行epoch迭代
        ...
        if epoch%(check_step)==0 and loss[epoch-1-loadepochs]<Loss_reshold:
