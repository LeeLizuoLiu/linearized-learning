...
def ResLoss_upw(x,bdry_x,f,divu_RHS,divf,bdry_velocity,beta,lamda,model_list,epoch):
    ...
    grad_u1_2 = torch.sum(torch.square(grad_u1_new[0]),dim=1)
    grad_u2_2 = torch.sum(torch.square(grad_u2_new[0]),dim=1)

    grad_p = torch.autograd.grad(interior_p_predict, x,  create_graph=True, grad_outputs=[torch.ones_like(interior_p_predict)])

    u_grad_u1 = interior_u_predict_new[:,0]*torch.sum(interior_u_predict_new*grad_u1_old[0], dim=1)
    u_grad_u2 = interior_u_predict_new[:,1]*torch.sum(interior_u_predict_new*grad_u2_old[0], dim=1)

    divu = grad_u1_new[0][:,0]+grad_u2_new[0][:,1]

    div_grad_p =  -torch.sum(torch.square(grad_p[0]),dim=1) +interior_p_predict*2*(-grad_u2_new[0][:,1]*grad_u1_new[0][:,0]+grad_u1_new[0][:,1]*grad_u2_new[0][:,0])
 
    loss_function = nn.MSELoss()
    
    loss1 = loss_function(u_grad_u1+global_nu*grad_u1_2+interior_u_predict_new[:,0]*grad_p[0][:,0], interior_u_predict_new[:,0]*f[:,0])
    loss2 = loss_function(u_grad_u2+global_nu*grad_u2_2+interior_u_predict_new[:,1]*grad_p[0][:,1], interior_u_predict_new[:,1]*f[:,1])
    loss4 = loss_function(bdry_u_predict, bdry_velocity[:, 0:2])
    loss5 = loss_function(divu, divu_RHS)
    loss6 = loss_function(div_grad_p,divf*interior_p_predict)
