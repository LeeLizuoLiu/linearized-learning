...
import sys
sys.path.append("..")
from utils.utils import FullyConnectedNet,load_mesh,load_pretrained_model,evaluate,plot_contourf_vorticity,DatasetFromTxt,generate_data,StatGrad,genData_test
   ...
   # Training settings
    ...
    for epoch in range(loadepochs+1, args.epochs + 1): # 循环调用train() and test()进行epoch迭代
        if  coarse_loss< 3000 and epoch%1==0 :
            x_int, inter_target, xlxb, ulub, xr_dirichlet, pr  = genData_test(len_inte_data,len_bound_data)
        ...
        loss[epoch-loadepochs-1],lamda_temp, bound, res, coarse_loss=train(args, model_list, device, interior_training_data_loader, 
                                                   ...
                                                   coarse_data_loader,
                                                   optimizer1,optimizer2,optimizer3, epoch, lamda,beta,gamma,CollectNorm)

        if epoch%50==0:
            CollectNorm = StatGrad(model_list[0],CollectNorm)
